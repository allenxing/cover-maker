...
export const useKonvaSnapping = (params) => {
  ...
  const getSnappingPoints = (e) => {
    ...
    if (snapToShapes) {
      stage.children.forEach((layer) => {
        // find group
        let nodes = layer.children;
        const groupNode = nodes.find((item) => item.getType() === "Group");
        if (groupNode) {
  ...
  const handleDragging = (e) => {
    ...
    const groupNode = e.target.findAncestors("Group")[0];
    ...
    guideLinesX.forEach((line) => {
      if (
        ...
        Math.round(
          e.target.getClientRect().x - line + e.target.getClientRect().width / 2
        ) === 0 ||
        Math.round(
  ...
  const handleResizing = (e) => {
    ...
    if (
      !e.currentTarget.keepRatio() ||
      (e.currentTarget.keepRatio() &&
        !!!oppositeAnchors[e.currentTarget._movingAnchorName])
    ) {
      ...
      e.currentTarget.anchorDragBoundFunc((oldAbsPos, newPos, event) => {
        ...
        const oppositeElement = e.currentTarget.findOne(
          `.${oppositeAnchorName}`
        );
